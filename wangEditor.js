...
UploadImg.prototype = {
    ...
    insertLinkVideo: function insertLinkVideo(link) {
        ...
        // editor.cmd.do('insertHTML', '<p align="center"><iframe src="' + link + '" style="width:650px;height: 485px" frameborder="0"></iframe></p>');
        editor.cmd.do('insertHTML', '<video controls="controls" src="'+link+'" width="640" height="360" style="width:640px;height:360px;max-width: 100%"> </video>');
    },
    ...
    uploadVideo: function uploadVideo(files) {
        ...
        if (uploadVideoServer && typeof uploadVideoServer === 'string') {
            ...
            // 监控 progress
            if (xhr.upload) {
                editor.cmd.do('insertHTML', '<div id="bar"></div>');
                var percent = 0;
                var bar = new ProgressBar.Line('#bar', {
                    ...
                    step: (state, bar) => {
                        bar.setText(Math.ceil(percent * 100) + '%');
                    }
                });
                xhr.upload.onprogress = function (e) {
